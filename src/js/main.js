...
const showCurrentWeather = (data) => {
  ...
  document.querySelector(".sun__sunrise").textContent = `${
    Number(sunrise.slice(0, 2)) + 2
  }:${sunrise.slice(-2)}`;
  document.querySelector(".sun__sunset").textContent = `${
    Number(sunset.slice(0, 2)) + 2
  }:${sunset.slice(-2)}`;
  document.querySelector(
    ".current-weather__icon"
  ).src = `dist/images/icons/${weather.icon}.svg`;
  document.querySelector(".current-weather__name").textContent =
    weather.description;
  document.querySelector(
    ".current-weather__temp"
  ).innerHTML = `${temp} <sup>o</sup>C`;
  document.querySelector(".current-weather__feels-like").textContent =
    feelsLikeTemp;
  ...
  document.querySelector(".current-weather__visibility").textContent =
    visibility;
...
const showForecast = (data) => {
  ...
  dailyForecast.forEach((item) => {
    ...
    forecastBox.querySelector(".forecast__date").innerText = item.valid_date
      .substr(5)
      .split("-")
      .reverse()
      .join(".");
    forecastBox.querySelector(
      ".forecast__icon"
    ).src = `dist/images/icons/${item.weather.icon}.svg`;
    forecastBox.querySelector(".forecast__name").innerText = firstCapitalLetter(
      item.weather.description
    );
    forecastBox.querySelector(
      ".forecast__temp"
    ).innerHTML = `${item.temp.toFixed(0)} <sup>o</sup>C`;
  });
};
...
const startApp = async () => {
  const data =
    clickSrc == "searchBtn" ? await getCityName() : await getGeoLocation();
